...
describe("HiveRepository PostgreSQL integration", () => {
  ...
  it("creates and maps a hive", async () => {
    ...
    const created = await repository.create({ accountId, name, status: true });
  ...
  it("lists only the account hives in deterministic order", async () => {
    ...
    const hives = await repository.findByAccountId(ownerAccountId);
  ...
  it("updates only an account-owned hive", async () => {
    ...
    const updated = await repository.update({ accountId: ownerAccountId, hiveId, name: "Updated hive", status: false });
    const foreignUpdate = await repository.update({ accountId: otherAccountId, hiveId, name: "Stolen hive", status: true });
    const missingUpdate = await repository.update({ accountId: ownerAccountId, hiveId: randomUUID(), name: "Missing hive", status: true });
    const persisted = await database.query<{ name: string; status: boolean }>(
      "SELECT name, status FROM hives WHERE hive_id = $1",
      [hiveId],
    );

    // then only the owner's update succeeds and the stored row is not mutated cross-account
    expect(updated).toMatchObject({ hiveId, accountId: ownerAccountId, name: "Updated hive", status: false });
    expect(foreignUpdate).toBeNull();
    expect(missingUpdate).toBeNull();
    expect(persisted.rows[0]).toEqual({ name: "Updated hive", status: false });
  });
  ...
  it("maps case-insensitive create and update conflicts", async () => {
    ...
    const first = await repository.create({ accountId: firstAccountId, name: `North ${marker}`, status: true });
    const updateCandidate = await repository.create({ accountId: firstAccountId, name: `South ${marker}`, status: true });
    ...
    const [duplicateCreate, duplicateUpdate, crossAccountCreate] = await Promise.allSettled([
      repository.create({ accountId: firstAccountId, name: `nOrTh ${marker}`, status: false }),
      repository.update({
      ...
      repository.create({ accountId: secondAccountId, name: `NORTH ${marker}`, status: false }),
    ]);
  ...
  async function insertHive(
    ...
    await database.query(
      `INSERT INTO hives (hive_id, account_id, name, status, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $5)`,
      [hiveId, accountId, name, status, createdAt],
    );
    return hiveId;
  }
});
